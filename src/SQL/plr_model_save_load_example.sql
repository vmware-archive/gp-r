...
----------------------------------------------------------------------------------	
	...
	drop table if exists gpdemo.patient_history_all cascade;
	create table gpdemo.patient_history_all as (
		select *, random() as TEMP_FOR_SPLITTING from gp_patient_history_1million where infection_cost > 0
	...
	create table gpdemo.patient_history_test as (
        ...
        select * from gpdemo.patient_history_all limit 100;
...
--1) Use case :  PL/R model save, load and score example
	...
	select 'mdl_binary_size' as param, length(mdl) as size
	from
	(
	...
	select (t).params, (t).estimate,(t).std_Error, (t).z_value float, (t).pr_gr_z 
	from 
	(
